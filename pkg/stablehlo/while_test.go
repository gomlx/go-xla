...
func TestWhile(t *testing.T) {
	t.Run("count from 0 to 10", func(t *testing.T) {
		...
		counter := must1(fn.ConstantFromScalar(int32(0)))

		// Condition function: counter < 10
		condFn := fn.Closure()
		condCounter := must1(condFn.Input(counter.Shape()))
		limit := must1(condFn.ConstantFromScalar(int32(10)))
		cond := must1(Compare(condCounter, limit, types.CompareLT, types.CompareSigned))
		...
		bodyFn := fn.Closure()
		bodyCounter := must1(bodyFn.Input(counter.Shape()))
		one := must1(bodyFn.ConstantFromScalar(int32(1)))
		nextCounter := must1(Add(bodyCounter, one))
	...
	t.Run("multiple state values", func(t *testing.T) {
		...
		counter := must1(fn.ConstantFromScalar(int32(0)))
		sum := must1(fn.ConstantFromScalar(int32(0)))

		// Condition function: counter < 5
		condFn := fn.Closure()
		condCounter := must1(condFn.Input(counter.Shape()))
		condSum := must1(condFn.Input(sum.Shape()))
		_ = condSum // not used in condition
		limit := must1(condFn.ConstantFromScalar(int32(5)))
		cond := must1(Compare(condCounter, limit, types.CompareLT, types.CompareSigned))
		if err := condFn.Return(cond); err != nil {
			t.Fatalf("condFn.Return: %v", err)
		}
		...
		bodyFn := fn.Closure()
		bodyCounter := must1(bodyFn.Input(counter.Shape()))
		bodySum := must1(bodyFn.Input(sum.Shape()))
		one := must1(bodyFn.ConstantFromScalar(int32(1)))
		nextCounter := must1(Add(bodyCounter, one))
		nextSum := must1(Add(bodySum, nextCounter))
	...
	t.Run("body output shape mismatch", func(t *testing.T) {
		...
		expanded1 := must1(Reshape(bodyCounter, shapes.Make(dtypes.Int32, 1)))
		expanded2 := must1(Reshape(bodyCounter, shapes.Make(dtypes.Int32, 1)))
		vector := must1(Concatenate(0, expanded1, expanded2))
	...
	t.Run("float64 loop", func(t *testing.T) {
		...
		// Initial state: x = 0.0
		x := must1(fn.ConstantFromScalar(0.0))

		// Condition function: x < 1.0
		condFn := fn.Closure()
		condX := must1(condFn.Input(x.Shape()))
		limit := must1(condFn.ConstantFromScalar(1.0))
		cond := must1(Compare(condX, limit, types.CompareLT, types.CompareFloat))
		if err := condFn.Return(cond); err != nil {
			t.Fatalf("condFn.Return: %v", err)
		}
		...
		bodyFn := fn.Closure()
		bodyX := must1(bodyFn.Input(x.Shape()))
		delta := must1(bodyFn.ConstantFromScalar(0.1))
		nextX := must1(Add(bodyX, delta))
	...
	t.Run("tensor state", func(t *testing.T) {
		...
		initialVec := must1(fn.ConstantFromFlatAndDimensions([]int32{0, 0, 0}, 3))

		// Condition function: check if first element < 5
		condFn := fn.Closure()
		condVec := must1(condFn.Input(initialVec.Shape()))
		firstElem := must1(Slice(condVec, []int{0}, []int{1}, []int{1}))
		firstScalar := must1(Reshape(firstElem, shapes.Make(dtypes.Int32)))
		limit := must1(condFn.ConstantFromScalar(int32(5)))
		cond := must1(Compare(firstScalar, limit, types.CompareLT, types.CompareSigned))
		if err := condFn.Return(cond); err != nil {
			t.Fatalf("condFn.Return: %v", err)
		}
		...
		bodyFn := fn.Closure()
		bodyVec := must1(bodyFn.Input(initialVec.Shape()))
		ones := must1(bodyFn.ConstantFromFlatAndDimensions([]int32{1, 1, 1}, 3))
		nextVec := must1(Add(bodyVec, ones))
